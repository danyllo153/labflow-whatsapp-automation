...
Lista completa com formato e exemplo de cada comando: docs/comandos.md.
Arquitetura
```mermaid
flowchart LR
...
Detalhes em docs/arquitetura.md e docs/deploy-vps.md.
Stack
...
Revisão do JSON depois de mudanças estruturais. Religar conexões à mão no editor já introduziu bugs que só apareceram na revisão do export (ver Bug 15 em troubleshooting).
Roadmap
...
Documentação
Documento	Conteúdo
comandos.md	Todos os comandos do bot, cargos e permissões
arquitetura.md	Nodes do workflow, abas da planilha e decisões de design de cada etapa
troubleshooting.md	15 bugs reais: sintoma, causa raiz, solução e lição
deploy-vps.md	Infraestrutura no VPS: rede, segredos, acesso SSH, migração
